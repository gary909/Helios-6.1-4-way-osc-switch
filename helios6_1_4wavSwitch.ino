...
/*    v6.1
 *    Added switch to choose between 4 osc waves forms - second switch on d pin 5
 *     
 *    v5.1
 ...
 *     
 *    See older code for full changes (it was getting rather long!)
*/
...
#include <tables/sin2048_int8.h>                // for Drone alone        ***New waveforms shared with Helios***
#include <tables/triangle_valve_2_2048_int8.h>  // for Drone alone
#include <tables/triangle2048_int8.h> // For Helios                       ***New waveforms***
...
#define WAVE_SWITCH 2 // switch for switching waveforms 1 - 2
#define WAVE_SWITCH2 5 // switch for switching waveforms 3 - 4        ***New waveforms***
...
Oscil <SIN2048_NUM_CELLS, AUDIO_RATE> oscil3; // Sin Wave               ***New waveforms***
Oscil <TRIANGLE2048_NUM_CELLS, AUDIO_RATE> oscil4; // Tri Wave          ***New waveforms***
...
int oldOsc2 = 99;  // remember the previous value of the second wave switches, so we know if it's changed 
...
//****************Set toggle switches***************************************************************************** 
  ...
  pinMode(5, INPUT_PULLUP); // Pin five to switch, high switch goes to 5v, low to ground. Selects second wave in Helios  ***New waveforms***
//****************End*********************************************************************************************
...
if (synthMode) // If switch is set to high, run this portion of code
 ...
 if ((abs(oldAtk-atkVal)>6) || (abs(oldDky-dkyVal)>6) || (abs(oldSus-susVal)>6) || (abs(oldRel-relVal)>6))  //see if pots have been moved, but ignore noise
  ...
  int sensorVal5 = digitalRead(WAVE_SWITCH2); // read the switch position value into a  variable
  if (sensorVal2 != oldOsc || sensorVal5 != oldOsc2) { // only set the oscillator if the wave switch has changed value ****WORK ON THIS****  ADD  second val hinto NEW IF
    if (sensorVal2 == HIGH && sensorVal5 == HIGH)   // If switch is set to high, run this portion of code
      {
        oscil1.setTable(SAW2048_DATA);
      }
    else if (sensorVal2 == HIGH && sensorVal5 == LOW)  // If switch not set to high, run this portion of code instead
      {
        oscil1.setTable(SIN2048_DATA);
      }
    else if (sensorVal2 == LOW && sensorVal5 == HIGH)  // If switch not set to high, run this portion of code instead
      {
        oscil1.setTable(TRIANGLE2048_DATA);
      }
      else if (sensorVal2 == LOW && sensorVal5 == LOW)  // If switch not set to high, run this portion of code instead
      {
        oscil1.setTable(SQUARE_NO_ALIAS_2048_DATA);
      }
     oldOsc = sensorVal2; // remember this value so we can check if it changes
     oldOsc2 = sensorVal5; // remember this value so we can check if it changes
  }
  //**********************************END HELIOS CONTROLS***************************************************************
